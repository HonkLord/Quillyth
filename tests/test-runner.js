...
class TestRunner {
  ...
  async testPlayerArcsAPI() {
    await this.test("Player Arcs API - GET", async () => {
      ...
      const playerArcs = await this.apiRequest(`/api/characters/${playerId}/arcs`);
      this.assert(
  ...
  async testPlayerArcsUpdate() {
    await this.test("Player Arcs API - UPDATE", async () => {
      const characters = await this.apiRequest("/api/characters");
      this.assert(characters.players && characters.players.length > 0, "Prerequisite: At least one player character must exist.");
      ...
      const result = await this.apiRequest(`/api/characters/${testPlayerId}/arcs`, {
        method: "POST",
        body: JSON.stringify(arcData),
      });

      this.assert(result.success, "Should successfully create a new player arc.");
  ...
  async runAllTests() {
    ...
    try {
      ...
      console.error("❌ CRITICAL: Could not fetch current campaign. Most tests will fail.", error);
      this.campaignId = 'campaign-4-old-cistern'; // Fallback to a known ID if needed
      console.log(`\n⚠️  Using fallback campaign ID: ${this.campaignId}`);
    }
