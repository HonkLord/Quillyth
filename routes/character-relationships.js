...
module.exports = (db) => {
  ...
  router.get("/", (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch character relationships" });
    }
  });
  ...
  router.post("/", (req, res) => {
    try {
      ...
      // Require campaign_id from either query or body, no fallback
      const campaignId =
        req.query.campaign_id || relationshipMatrix.campaign_id;
      if (!campaignId) {
        return res.status(400).json({ error: "Campaign ID is required" });
      }
      // Check if this is a single relationship or bulk data
      if (
        relationshipMatrix.campaign_id &&
        relationshipMatrix.from_character_id
      ) {
        ...
        if (
          ...
          return res.status(400).json({
            error:
              "Campaign ID, from/to character IDs, and relationship type are required",
          });
        }
        const relationshipId = `rel_${from_character_id}_${to_character_id}_${randomUUID()}`;
        const result = db
          .prepare(
      ...
      // Wrap bulk operations in a transaction for data integrity
      const transaction = db.transaction(() => {
        ...
        let insertedCount = 0;
        Object.entries(relationshipMatrix).forEach(
          ([fromCharId, relationships]) => {
            Object.entries(relationships).forEach(
              ([toCharId, relationship]) => {
                const relationshipId = `rel_${fromCharId}_${toCharId}_${randomUUID()}`;
                insertStmt.run(
                  relationshipId,
                  campaignId,
                  fromCharId,
  ...
  router.put("/:fromId/:toId", (req, res) => {
    try {
      ...
      if (result.changes === 0) {
        return res
          .status(404)
          .json({ error: "Character relationship not found" });
      }
      ...
      res
        .status(500)
        .json({ error: "Failed to update character relationship" });
    }
  });
  ...
  router.delete("/:fromId/:toId", (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to delete character relationship" });
    }
  });
